          ...
          <div className="flex h-screen overflow-hidden">
            <aside className="w-80 bg-white border-r border-slate-100 p-8 flex flex-col gap-10 hidden md:flex h-full overflow-y-auto shrink-0 z-50">
              <div className="flex items-center gap-4 px-2">
                <div className="bg-slate-900 p-3 rounded-[1.25rem] shadow-xl"><Trash2 className="text-white w-6 h-6" /></div>
                <div>
                  <h1 className="text-xl font-black italic tracking-tighter uppercase leading-none text-slate-900">SWM PRO</h1>
              ...
              <nav className="flex flex-col gap-8">
                {menuGroups.map((group, idx) => (
                  <div key={idx} className="space-y-3">
                    <p className="text-[10px] font-black uppercase text-slate-300 tracking-[0.2em] px-4">{group.title}</p>
                    <div className="flex flex-col gap-1">
                      {group.items.map((item) => (
                        <Link key={item.path} href={item.path} className={`flex items-center gap-4 px-6 py-4 rounded-[1.8rem] font-bold text-[11px] uppercase tracking-wider transition-all duration-300 group ${pathname === item.path ? 'bg-sky-500 text-white shadow-xl scale-105 translate-x-2' : 'text-slate-400 hover:bg-slate-50 hover:text-sky-500'}`}>
                          <item.icon className="w-4 h-4 transition-transform group-hover:scale-110" /> {item.name}
                        </Link>
                      ))}
                    </div>
                  </div>
            ...
            <main className="flex-1 bg-slate-50 relative h-full overflow-hidden">{children}</main>
          </div>
        </Providers>
      </body>
    </html>
